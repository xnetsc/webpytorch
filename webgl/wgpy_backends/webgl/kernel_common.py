...
def parse_in_params(in_params: str) -> List[InParam]:
    ...
    for param in in_params.split(","):
        m = re.match(
            "^\\s*(?:(raw|rawnd)\\s+)?(float|int|uint|[A-Z])\\s+([a-zA-Z][a-zA-Z0-9_]*)\\s*$",
            param,
        )
...
def parse_out_params(out_params: str) -> OutParam:
    ...
    m = re.match(
        "^\\s*(float|int|uint|[A-Z])\\s+([a-zA-Z][a-zA-Z0-9_]*)\\s*$", out_params
    )
...
def make_input_reduction_uniform(input_shape: Tuple[int, ...]):
    uniforms = []
    uniforms.append(
        {"type": "int", "name": f"_in_ind_size", "value": int(np.prod(input_shape))}
    )
    return uniforms
...
def make_output_uniform(param: OutParam, webgl_array: ndarray, reduction: bool):
    ...
    uniforms.append(
        {
            "type": "int",
            "name": f"_{name}_texture_w",
            "value": webgl_array.buffer.texture_shape.width,
        }
    )
    if webgl_array.buffer.texture_shape.dim == "2DArray":
        uniforms.append(
            {
                "type": "int",
                "name": f"_{name}_texture_h",
                "value": webgl_array.buffer.texture_shape.height,
            }
        )

    for d in range(webgl_array.ndim):
        uniforms.append(
            {"type": "int", "name": f"_{name}_shape_{d}", "value": webgl_array.shape[d]}
        )
    if reduction:
        # cupy's _out_ind.size()
        uniforms.append(
            {"type": "int", "name": f"_out_ind_size", "value": webgl_array.size}
        )
    else:
        # elementwise
        # cupy's _ind.size()
        uniforms.append(
            {"type": "int", "name": f"_ind_size", "value": webgl_array.size}
        )
    return uniforms
